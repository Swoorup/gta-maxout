...
bool _cdecl CCarCtrl::PickNextNodeToFollowPath(CVehicle* pVehicle) {
    ...
    int nPreviousNextNodeIndex = pVehicle->stAutopilot.m_dwNextNode;
	int m_beginRouteStep = pVehicle->stAutopilot.m_nRouteListStep;
    //if(pVehicle->stAutopilot.m_nRouteListStep == 0)  // SOMEFUNCTIONS CAUSES THE ERROR DOWN BELOW
	{
        THEPATHS->DoPathSearch(0, pVehicle->mat.vPos,
            pVehicle->stAutopilot.m_dwNextNode,
            pVehicle->stAutopilot.m_vecTargetPoint,
            pVehicle->stAutopilot.m_pIntermediateRouteList,
            &pVehicle->stAutopilot.m_nRouteListStep,
            8,
            pVehicle,
            0,
            999999.88f,
        ...
        if(pVehicle->stAutopilot.m_nRouteListStep < 2)
            return true;
        
		// remove the first node since it is already known
        pVehicle->stAutopilot.RemoveOnePathNode();
    }

    pVehicle->stAutopilot.m_dwPrevNode = pVehicle->stAutopilot.m_dwCurrentNode;
    pVehicle->stAutopilot.m_dwCurrentNode = pVehicle->stAutopilot.m_dwNextNode;
    /*if(pVehicle->stAutopilot.m_pIntermediateRouteList[0] < &THEPATHS->m_AttachedPaths[0] || pVehicle->stAutopilot.m_pIntermediateRouteList[0] > &THEPATHS->m_AttachedPaths[THEPATHS->m_nAttachedNodes -1]){
        //sometimes the m_pIntermediateRouteList contains inappropriate value which is below m_AttachedPaths[0] temp fix
        char buf[256];
        sprintf(buf, "Intermediate Address: 0x%X \nStupidNode: 0x%X", pVehicle->stAutopilot.m_pIntermediateRouteList, pVehicle->stAutopilot.m_pIntermediateRouteList[0]);
    ...
    pVehicle->stAutopilot.m_dwNextNode = pVehicle->stAutopilot.m_pIntermediateRouteList[0] - &THEPATHS->m_AttachedPaths[0];
	if (pVehicle->stAutopilot.m_dwNextNode <0)
		
	{
		short val = (short)pVehicle->stAutopilot.m_pIntermediateRouteList[0];
		int iVal = (int)pVehicle->stAutopilot.m_pIntermediateRouteList[0];
		char buf[256];
		sprintf(buf, "ERROR: function began with %d as Steps", m_beginRouteStep);
		MessageBoxA(NULL, buf, NULL, 0);
	}
    pVehicle->stAutopilot.RemoveOnePathNode();
    pVehicle->stAutopilot.m_dwCurrentSpeedScaleFactor += pVehicle->stAutopilot.m_dwNextSpeedScaleFactor;
    pVehicle->stAutopilot.m_dwPrevCarLinkNode = pVehicle->stAutopilot.m_dwCurrentCarLinkNode;
    pVehicle->stAutopilot.m_dwCurrentCarLinkNode = pVehicle->stAutopilot.m_dwNextCarLinkNode;
    pVehicle->stAutopilot.m_bytePrevDirectionScale = pVehicle->stAutopilot.m_byteCurrentDirectionScale;
    pVehicle->stAutopilot.m_byteCurrentDirectionScale = pVehicle->stAutopilot.m_byteNextDirectionScale;
    pVehicle->stAutopilot.m_byteCurrentLanes = pVehicle->stAutopilot.m_byteNextLanes;

    int nCurrentNodeRouteInfo = THEPATHS->m_AttachedPaths[pVehicle->stAutopilot.m_dwCurrentNode].wRouteInfoIndex;
    int nConnectedNodeIndex = 0;
    
	/// MAJOR BUG WHAT IF NOT EVER FOUND, COULD GO UP TO HUGE LOOPS
	while(pVehicle->stAutopilot.m_dwNextNode != (THEPATHS->m_infoConnectedNodes[nCurrentNodeRouteInfo + nConnectedNodeIndex] & CPathFind::em_infoConnectedNodesNODEINDEXONLY)) 
	{
        nConnectedNodeIndex++;	
    }

    pVehicle->stAutopilot.m_dwNextCarLinkNode = THEPATHS->m_InfoCarPathLinks[nCurrentNodeRouteInfo + nConnectedNodeIndex];

    int nRequiredLanes;
    if(nPreviousNextNodeIndex >= pVehicle->stAutopilot.m_dwNextNode) {
        pVehicle->stAutopilot.m_byteNextDirectionScale = 1;
        nRequiredLanes = THEPATHS->m_CarPathLinks[pVehicle->stAutopilot.m_dwNextCarLinkNode].bitRightLanes;
    }
    else {
        pVehicle->stAutopilot.m_byteNextDirectionScale = -1;
        nRequiredLanes = THEPATHS->m_CarPathLinks[pVehicle->stAutopilot.m_dwNextCarLinkNode].bitLeftLanes;
    }

    CVector2D v2dCurrentDirection, v2dNextDirection;
    int dwCurrentDetachedIndex = pVehicle->stAutopilot.m_dwCurrentCarLinkNode;
    int dwNextDetachedIndex = pVehicle->stAutopilot.m_dwNextCarLinkNode;
    v2dCurrentDirection.fX = (float)pVehicle->stAutopilot.m_byteCurrentDirectionScale * (float)(THEPATHS->m_CarPathLinks[dwCurrentDetachedIndex].NormalVecX) / 100.0f;
    v2dCurrentDirection.fY = (float)pVehicle->stAutopilot.m_byteCurrentDirectionScale * (float)(THEPATHS->m_CarPathLinks[dwCurrentDetachedIndex].NormalVecY) / 100.0f;
    v2dNextDirection.fX = (float)pVehicle->stAutopilot.m_byteNextDirectionScale * (float)(THEPATHS->m_CarPathLinks[dwNextDetachedIndex].NormalVecX) / 100.0f;
    v2dNextDirection.fY = (float)pVehicle->stAutopilot.m_byteNextDirectionScale * (float)(THEPATHS->m_CarPathLinks[dwNextDetachedIndex].NormalVecY) / 100.0f;
    ...
    if(nRequiredLanes >= 0) {
        ...
        if(fDisplacement > 49.0f && !(rand() & 0x600)) {
            ...
            if(bRandTest) {
                pVehicle->stAutopilot.m_byteNextLanes++;
            }
            else {
                pVehicle->stAutopilot.m_byteNextLanes--;
            }
        }
        if(pVehicle->stAutopilot.m_byteNextLanes >= nRequiredLanes - 1) {
            pVehicle->stAutopilot.m_byteNextLanes = nRequiredLanes - 1;
        }
        if(pVehicle->stAutopilot.m_byteNextLanes <= 0) {
            pVehicle->stAutopilot.m_byteNextLanes = 0;
        }
    }
    else {
        pVehicle->stAutopilot.m_byteNextLanes = pVehicle->stAutopilot.m_byteCurrentLanes;
    }
    if((pVehicle->stAutopilot.m_flags >> 3) & 1) {
        pVehicle->stAutopilot.m_byteNextLanes = 0;
    }
    float fCurrentLaneLength = (THEPATHS->m_CarPathLinks[dwCurrentDetachedIndex].OneWayLaneOffset() + (float)(pVehicle->stAutopilot.m_byteCurrentLanes)) * 5.0f;
    float fNextLaneLength = (THEPATHS->m_CarPathLinks[dwNextDetachedIndex].OneWayLaneOffset() + (float)(pVehicle->stAutopilot.m_byteNextLanes)) * 5.0f;
    ...
    pVehicle->stAutopilot.m_dwNextSpeedScaleFactor = (signed int)(CCurves::CalcSpeedScaleFactor(&v2dCurrentTest, &v2dNextTest, v2dCurrentDirection, v2dNextDirection) * 1000.0f / pVehicle->stAutopilot.m_fCurrentSpeed);
    if(pVehicle->stAutopilot.m_dwNextSpeedScaleFactor <= 10) {
        pVehicle->stAutopilot.m_dwNextSpeedScaleFactor = 10;
    }
    return false;
}
...
void _cdecl CCarCtrl::PickNextNodeToChaseCar(CVehicle* pVehicle, float fTargetX, float fTargetY, CVehicle* pTargetVehicle) {
	...
	int dwMainNode = pVehicle->stAutopilot.m_dwCurrentNode;
	int dwNextNode = pVehicle->stAutopilot.m_dwNextNode;
	...
	if(sRouteStep != 1 && sRouteStep != 2 || pInstantRoutes[0] == &THEPATHS->m_AttachedPaths[dwNextNode]) 
	{
		...
		{
			CVector2D v2dVehiclePos(CVector2D(fTargetX, fTargetY) - CVector2D(pVehicle->mat.vPos));
	...
	pVehicle->stAutopilot.m_dwPrevNode = pVehicle->stAutopilot.m_dwCurrentNode;
	pVehicle->stAutopilot.m_dwCurrentNode = pVehicle->stAutopilot.m_dwNextNode;
	pVehicle->stAutopilot.m_dwNextNode = nNodeWithSmallestDiversion;
	pVehicle->stAutopilot.m_dwCurrentSpeedScaleFactor += pVehicle->stAutopilot.m_dwNextSpeedScaleFactor;
	pVehicle->stAutopilot.m_dwPrevCarLinkNode = pVehicle->stAutopilot.m_dwCurrentCarLinkNode;
	pVehicle->stAutopilot.m_dwCurrentCarLinkNode = pVehicle->stAutopilot.m_dwNextCarLinkNode;
	pVehicle->stAutopilot.m_bytePrevDirectionScale = pVehicle->stAutopilot.m_byteCurrentDirectionScale;
	pVehicle->stAutopilot.m_byteCurrentDirectionScale = pVehicle->stAutopilot.m_byteNextDirectionScale;
	pVehicle->stAutopilot.m_byteCurrentLanes = pVehicle->stAutopilot.m_byteNextLanes;
	pVehicle->stAutopilot.m_dwNextCarLinkNode = THEPATHS->m_InfoCarPathLinks[nFoundConnectedBit + THEPATHS->m_AttachedPaths[dwNextNode].wRouteInfoIndex];
	
	int nRequiredLanes;
	if(dwNextNode >= pVehicle->stAutopilot.m_dwNextNode) 
	{
		pVehicle->stAutopilot.m_byteNextDirectionScale = 1;
		nRequiredLanes = THEPATHS->m_CarPathLinks[pVehicle->stAutopilot.m_dwNextCarLinkNode].bitRightLanes;
	}
	...
	int dwCurrentDetachedNode = pVehicle->stAutopilot.m_dwCurrentCarLinkNode;
	int dwNextDetachedNode = pVehicle->stAutopilot.m_dwNextCarLinkNode;
	CVector2D v2dCurrentDirection, v2dNextDirection;
	v2dCurrentDirection.fX = (float)(pVehicle->stAutopilot.m_byteCurrentDirectionScale) * (float)(THEPATHS->m_CarPathLinks[dwCurrentDetachedNode].NormalVecX) / 100.0f;
	v2dCurrentDirection.fY = (float)(pVehicle->stAutopilot.m_byteCurrentDirectionScale) * (float)(THEPATHS->m_CarPathLinks[dwCurrentDetachedNode].NormalVecY) / 100.0f;
	v2dNextDirection.fX = (float)(pVehicle->stAutopilot.m_byteNextDirectionScale) * (float)(THEPATHS->m_CarPathLinks[dwNextDetachedNode].NormalVecX) / 100.0f;
	v2dNextDirection.fY = (float)(pVehicle->stAutopilot.m_byteNextDirectionScale) * (float)(THEPATHS->m_CarPathLinks[dwNextDetachedNode].NormalVecY) / 100.0f;
	...
	if(nRequiredLanes >= 0) 
	{
		...
		{
			int eAutoPilotBehaviour = pVehicle->stAutopilot.m_DriverBehaviour;
	...
	float fCurrentLaneLength = (THEPATHS->m_CarPathLinks[dwCurrentDetachedNode].OneWayLaneOffset() + (float)(pVehicle->stAutopilot.m_byteCurrentLanes)) * 5.0f;
	float fNextLaneLength = (THEPATHS->m_CarPathLinks[dwNextDetachedNode].OneWayLaneOffset() + (float)(pVehicle->stAutopilot.m_byteNextLanes)) * 5.0f;
	...
	pVehicle->stAutopilot.m_dwNextSpeedScaleFactor = (signed int)(CCurves::CalcSpeedScaleFactor(&v2dCurrentDetachedTest, &v2dNextDetachedTest, v2dCurrentDirection, v2dNextDirection) * 1000.0f/ pVehicle->stAutopilot.m_fCurrentSpeed);
	if(pVehicle->stAutopilot.m_dwNextSpeedScaleFactor <= 10) {
		pVehicle->stAutopilot.m_dwNextSpeedScaleFactor = 10;
	}
}
...
void _cdecl CCarCtrl::PickNextNodeRandomly(CVehicle* pVehicle) {
	...
	int dwPrevNextNode = pVehicle->stAutopilot.m_dwNextNode;
	int nNextConnectedSets = THEPATHS->m_AttachedPaths[dwPrevNextNode].bitnumberOfNodesConnected;
	int dwPrevMainNode = pVehicle->stAutopilot.m_dwCurrentNode;
	int dwNextDetachedNode = pVehicle->stAutopilot.m_dwNextCarLinkNode;
	...
	unsigned char byteAutoNextLane = pVehicle->stAutopilot.m_byteNextLanes;
	...
	while(nLoopConnectedBits < 15) {
		...
		pVehicle->stAutopilot.m_dwNextNode = THEPATHS->m_infoConnectedNodes[nRandomConnectedBit + THEPATHS->m_AttachedPaths[dwPrevNextNode].wRouteInfoIndex] & CPathFind::em_infoConnectedNodesNODEINDEXONLY;
		unsigned char bytePathDirection = CCarCtrl::FindPathDirection(dwPrevMainNode, dwPrevNextNode, pVehicle->stAutopilot.m_dwNextNode);
		...
		int dwNextDeltaNode = pVehicle->stAutopilot.m_dwNextNode;
		if(dwPrevMainNode != dwNextDeltaNode && (byteTestPathDirection & bytePathDirection)) {
			if(!(THEPATHS->m_AttachedPaths[dwNextDeltaNode].bitUnknownFlag3) || THEPATHS->m_AttachedPaths[dwPrevMainNode].bitUnknownFlag3){
				...
				&& (!((pVehicle->stAutopilot.m_flags >> 2) & 1) || !(THEPATHS->m_AttachedPaths[dwNextDeltaNode].bitRestrictedAccess)
	...
	if(nLoopConnectedBits >= 15) {
		for(nLoopConnectedBits = 0; nLoopConnectedBits < 15; nLoopConnectedBits++) {
			...
			if(dwPrevMainNode != dwNextDeltaNode) {
				if(!(THEPATHS->m_AttachedPaths[dwNextDeltaNode].bitIsIgnoredNode) || THEPATHS->m_AttachedPaths[dwPrevMainNode].bitIsIgnoredNode) {
					if((!((pVehicle->stAutopilot.m_flags >> 2) & 1) || !(THEPATHS->m_AttachedPaths[dwNextDeltaNode].bitRestrictedAccess)
					|| THEPATHS->m_AttachedPaths[dwPrevMainNode].bitRestrictedAccess) && bIsOtherLaneNull == false) {
						break;
					}
				}
	...
	if(nLoopConnectedBits >= 15) {
		...
		if(nRandomConnectedBit < 999) {
			pVehicle->stAutopilot.m_dwNextNode = dwPrevMainNode;
		}
	}
	if(dwPrevMainNode == pVehicle->stAutopilot.m_dwNextNode) {
		pVehicle->bfTypeStatus = pVehicle->bfTypeStatus & 7 | 0x18;
		pVehicle->stAutopilot.m_DriverBehaviour = 1;
		pVehicle->stAutopilot.m_eSimpleAction = 0;
		pVehicle->stAutopilot.m_snGettingNewCommandTimeStamp = CGameVariables::GetTimeInMilliseconds();
		pVehicle->stAutopilot.m_snUnknownTimeStamp = CGameVariables::GetTimeInMilliseconds();
	}
	pVehicle->stAutopilot.m_dwCurrentSpeedScaleFactor += pVehicle->stAutopilot.m_dwNextSpeedScaleFactor;
	pVehicle->stAutopilot.m_dwPrevCarLinkNode = pVehicle->stAutopilot.m_dwCurrentCarLinkNode;
	pVehicle->stAutopilot.m_dwCurrentCarLinkNode = pVehicle->stAutopilot.m_dwNextCarLinkNode;
	...
	pVehicle->stAutopilot.m_byteCurrentDirectionScale = pVehicle->stAutopilot.m_byteNextDirectionScale;
	pVehicle->stAutopilot.m_byteCurrentLanes = pVehicle->stAutopilot.m_byteNextLanes;
	pVehicle->stAutopilot.m_dwNextCarLinkNode = THEPATHS->m_InfoCarPathLinks[nRandomConnectedBit + THEPATHS->m_AttachedPaths[dwPrevNextNode].wRouteInfoIndex];
	
	int dwNextLanes;
	if(dwPrevNextNode >= pVehicle->stAutopilot.m_dwNextNode) {
		pVehicle->stAutopilot.m_byteNextDirectionScale = 1;
		dwNextLanes = THEPATHS->m_CarPathLinks[pVehicle->stAutopilot.m_dwNextCarLinkNode].bitLeftLanes;
	}
	else {
		pVehicle->stAutopilot.m_byteNextDirectionScale = -1;
		dwNextLanes = THEPATHS->m_CarPathLinks[pVehicle->stAutopilot.m_dwNextCarLinkNode].bitRightLanes;
	}
	CVector2D v2dCurrentDirection, v2dNextDirection, v2dCurrentDetachedTest, v2dNextDetachedTest;
	v2dCurrentDirection.fX = (float)(pVehicle->stAutopilot.m_byteCurrentDirectionScale) * (float)(THEPATHS->m_CarPathLinks[pVehicle->stAutopilot.m_dwCurrentCarLinkNode].NormalVecX) / 100.0f;
	v2dCurrentDirection.fY = (float)(pVehicle->stAutopilot.m_byteCurrentDirectionScale) * (float)(THEPATHS->m_CarPathLinks[pVehicle->stAutopilot.m_dwCurrentCarLinkNode].NormalVecY) / 100.0f;
	v2dNextDirection.fX = (float)(pVehicle->stAutopilot.m_byteNextDirectionScale) * (float)(THEPATHS->m_CarPathLinks[pVehicle->stAutopilot.m_dwNextCarLinkNode].NormalVecX) / 100.0f;
	v2dNextDirection.fY = (float)(pVehicle->stAutopilot.m_byteNextDirectionScale) * (float)(THEPATHS->m_CarPathLinks[pVehicle->stAutopilot.m_dwNextCarLinkNode].NormalVecY) / 100.0f;
	
	if(dwNextLanes >= 0) {
		if(!(rand() & 0x600)) {
			CVector2D v2dCurrentAttached, v2dNextAttached;
			v2dCurrentAttached.fX = (float)(THEPATHS->m_AttachedPaths[pVehicle->stAutopilot.m_dwCurrentNode].wX) / 8.0f;
			v2dCurrentAttached.fY = (float)(THEPATHS->m_AttachedPaths[pVehicle->stAutopilot.m_dwCurrentNode].wY) / 8.0f;
			v2dNextAttached.fX = (float)(THEPATHS->m_AttachedPaths[pVehicle->stAutopilot.m_dwNextNode].wX) / 8.0f;
			v2dNextAttached.fY = (float)(THEPATHS->m_AttachedPaths[pVehicle->stAutopilot.m_dwNextNode].wY) / 8.0f;
		...
		if(pVehicle->stAutopilot.m_byteNextLanes >= (dwNextLanes - 1)) {
			pVehicle->stAutopilot.m_byteNextLanes = dwNextLanes - 1;
		}
		if(pVehicle->stAutopilot.m_byteNextLanes <= 0) {
			pVehicle->stAutopilot.m_byteNextLanes = 0;
		}
	...
	int dwTestCurrentDetachedNode = pVehicle->stAutopilot.m_dwCurrentCarLinkNode;
	int dwTestNextDetachedNode = pVehicle->stAutopilot.m_dwNextCarLinkNode;
	float fCurrentLaneLength = (THEPATHS->m_CarPathLinks[dwTestCurrentDetachedNode].OneWayLaneOffset() + (float)(pVehicle->stAutopilot.m_byteCurrentLanes)) * 5.0f;
	float fNextLaneLength = (THEPATHS->m_CarPathLinks[dwTestNextDetachedNode].OneWayLaneOffset() + (float)(pVehicle->stAutopilot.m_byteNextLanes)) * 5.0f;
...
bool _cdecl CCarCtrl::PickNextNodeAccordingStrategy(CVehicle* pVehicle) {
	pVehicle->stAutopilot.m_byteSpeedLimit = THEPATHS->m_AttachedPaths[pVehicle->stAutopilot.m_dwNextNode].bitSpeedLimit;
	switch(pVehicle->stAutopilot.m_DriverBehaviour) {
		case 2:
		case 4:
        {
		...
		case 15:
		case 17:
			PickNextNodeToChaseCar(pVehicle, pVehicle->pVehicleToRam->mat.vPos.fX, pVehicle->pVehicleToRam->mat.vPos.fY, pVehicle->pVehicleToRam);
			return false;
			break;
		case 8:
		case 12:
		...
		default:
        {
			...
			if(THEPATHS->m_AttachedPaths[pVehicle->stAutopilot.m_dwNextNode].bitHaveUnrandomizedVehClass) {
				if(pVehicle->nModelIndex == BOAT_RIO || pVehicle->nModelIndex == BOAT_TROPIC || pVehicle->nModelIndex == BOAT_MARQUIS) {
					pVehicle->stAutopilot.m_nMaxSpeed = 0;
				}
			}
			return false;
			break;
...
void _cdecl CCarCtrl::UpdateCarOnRails(CVehicle* pVehicle) {
	if(pVehicle->stAutopilot.m_eSimpleAction == 1) {
		pVehicle->vecMoveSpeed.fX = 0.0f;
		pVehicle->vecMoveSpeed.fY = 0.0f;
		pVehicle->vecMoveSpeed.fZ = 0.0f;
		pVehicle->stAutopilot.ModifySpeed(0.0f);
		if((int)CGameVariables::GetTimeInMilliseconds() > pVehicle->stAutopilot.m_simple_action_time) {
			pVehicle->stAutopilot.m_eSimpleAction = 0;
			pVehicle->stAutopilot.m_snGettingNewCommandTimeStamp = CGameVariables::GetTimeInMilliseconds();
			pVehicle->stAutopilot.m_snUnknownTimeStamp = CGameVariables::GetTimeInMilliseconds();
		}
		...
		if(CGameVariables::GetTimeInMilliseconds() >= (int64_t)(pVehicle->stAutopilot.m_dwNextSpeedScaleFactor + pVehicle->stAutopilot.m_dwCurrentSpeedScaleFactor)) {
			PickNextNodeAccordingStrategy(pVehicle);
		}
		if((pVehicle->bfTypeStatus >> 3) != 3) {
			float fTimeDelta = (float)(uint32_t)(CGameVariables::GetTimeInMilliseconds() - pVehicle->stAutopilot.m_dwCurrentSpeedScaleFactor) / (float)(pVehicle->stAutopilot.m_dwNextSpeedScaleFactor);
			int dwCurrentDetachedNode = pVehicle->stAutopilot.m_dwCurrentCarLinkNode;
			CVector2D vecCurrentDirection, vecNextDirection;
			vecCurrentDirection.fX = (float)(pVehicle->stAutopilot.m_byteCurrentDirectionScale) * (float)(THEPATHS->m_CarPathLinks[dwCurrentDetachedNode].NormalVecX) / 100.0f;
			vecCurrentDirection.fY = (float)(pVehicle->stAutopilot.m_byteCurrentDirectionScale) * (float)(THEPATHS->m_CarPathLinks[dwCurrentDetachedNode].NormalVecY) / 100.0f;
			int dwNextDetachedNode = pVehicle->stAutopilot.m_dwNextCarLinkNode;
			vecNextDirection.fX = (float)(pVehicle->stAutopilot.m_byteNextDirectionScale) * (float)(THEPATHS->m_CarPathLinks[dwNextDetachedNode].NormalVecX) / 100.0f;
			vecNextDirection.fY = (float)(pVehicle->stAutopilot.m_byteNextDirectionScale) * (float)(THEPATHS->m_CarPathLinks[dwNextDetachedNode].NormalVecY) / 100.0f;
			
			float fCurrentLane = (THEPATHS->m_CarPathLinks[dwCurrentDetachedNode].OneWayLaneOffset() + (float)(pVehicle->stAutopilot.m_byteCurrentLanes)) * 5.0f;
			float fNextLane = (THEPATHS->m_CarPathLinks[dwNextDetachedNode].OneWayLaneOffset() + (float)(pVehicle->stAutopilot.m_byteNextLanes)) * 5.0f;
			
			CVector2D vecCurrentLane, vecNextLane;
			vecCurrentLane.fX = (double)(((dwCurrentDetachedNode + pVehicle->uiPathMedianRand) & 7) - 3) * 0.0089999996f;
			vecCurrentLane.fY = (double)((((dwCurrentDetachedNode + pVehicle->uiPathMedianRand) >> 3) & 7) -3) * 0.0089999996f;
			vecNextLane.fX = (double)(((dwNextDetachedNode + pVehicle->uiPathMedianRand) & 7) - 3) * 0.0089999996f;
			vecNextLane.fY = (double)((((dwNextDetachedNode + pVehicle->uiPathMedianRand) >> 3) & 7) - 3) * 0.0089999996f;
			...
			CCurves::CalcCurvePoint(&vecCurrentDetachedTest, &vecNextDetachedTest, &vecCurrentLane, &vecNextLane, fTimeDelta, pVehicle->stAutopilot.m_dwNextSpeedScaleFactor, &vecGotoPoint, &vecRequiredVelocity);
			vecGotoPoint.fZ = 15.0f;
			DragCarToPoint(pVehicle, &vecGotoPoint);
			vecRequiredVelocity *= 0.016666668f;
			pVehicle->vecMoveSpeed.fX = vecRequiredVelocity.fX;
			pVehicle->vecMoveSpeed.fY = vecRequiredVelocity.fY;
			pVehicle->vecMoveSpeed.fZ = vecRequiredVelocity.fZ;
		}
	}
}
...
{
    DWORD _dwHookArgOne = pVehicle->wMissionValue; // seedvalue
    ...
    int nConnectedPointInfo = pThePaths->m_AttachedPaths[pVehicle->stAutopilot.m_dwCurrentNode].wRouteInfoIndex;
    int _nLoop = 0;

    for(int _nLoop = 0; _nLoop < 12 && pVehicle->stAutopilot.m_dwNextNode != (pThePaths->m_infoConnectedNodes[_nLoop + nConnectedPointInfo] & CPathFind::em_infoConnectedNodesNODEINDEXONLY); _nLoop++);
    pVehicle->stAutopilot.m_dwNextCarLinkNode = pThePaths->m_InfoCarPathLinks[_nLoop + nConnectedPointInfo];
    if(pVehicle->stAutopilot.m_dwCurrentNode >= pVehicle->stAutopilot.m_dwNextNode) {
        pVehicle->stAutopilot.m_byteNextDirectionScale = 1;
    }
    else {
        pVehicle->stAutopilot.m_byteNextDirectionScale = -1;
    ...
    int nStartNode = pVehicle->stAutopilot.m_dwCurrentNode;
    ...
    if(pThePaths->m_AttachedPaths[nStartNode].bitnumberOfNodesConnected == 1) {
        ...
        for(int j = 0; j < pThePaths->m_AttachedPaths[nStartNode].bitnumberOfNodesConnected; j++) {
            ...
            if(nConnectedNextNode == pVehicle->stAutopilot.m_dwNextNode) {
                continue;
            }
            ...
            CVector* vPosADDR = &pVehicle->mat.vPos;
    ...
    pVehicle->stAutopilot.m_dwCurrentCarLinkNode = nFoundDetachedNode;
    if((pThePaths->m_infoConnectedNodes[nFoundNode + pThePaths->m_AttachedPaths[nStartNode].wRouteInfoIndex] & CPathFind::em_infoConnectedNodesNODEINDEXONLY) >= nStartNode) {
        pVehicle->stAutopilot.m_byteCurrentDirectionScale = 1;
    }
    else {
        pVehicle->stAutopilot.m_byteCurrentDirectionScale = -1;
    }
...
void _cdecl CCarCtrl::JoinCarWithRoadSystem(CVehicle* pVehicle) {
	...
	pVehicle->stAutopilot.m_dwNextNode = 0;
	pVehicle->stAutopilot.m_dwCurrentNode = 0;
	pVehicle->stAutopilot.m_dwPrevNode = 0;
	pVehicle->stAutopilot.m_dwNextCarLinkNode = 0;
	pVehicle->stAutopilot.m_dwPrevCarLinkNode = 0;
	pVehicle->stAutopilot.m_dwCurrentCarLinkNode = 0;
	
	int nNodeClosestToDirection = pThePaths->FindNodeClosestToCoorsFavourDirection(pVehicle->mat.vPos,
																		0,
																		CVector2D(pVehicle->mat.vLookAt));
	...
	float fLookAtX = pVehicle->mat.vLookAt.fX;
	float fLookAtY = pVehicle->mat.vLookAt.fY;
	...
	pVehicle->stAutopilot.m_dwPrevNode = 0;
	pVehicle->stAutopilot.m_dwCurrentNode = nClosestNode;
	pVehicle->stAutopilot.m_dwNextNode = nNodeClosestToDirection;
	pVehicle->stAutopilot.m_nRouteListStep = 0;
	FindLinksToGoWithTheseNodes(pVehicle);
	pVehicle->stAutopilot.m_byteCurrentLanes = 0;
	pVehicle->stAutopilot.m_byteNextLanes = 0;
}
