...
CPathFind::CPathFind() 
{
    memset(m_infoConnectedNodes, 0, sizeof(m_infoConnectedNodes));
    ZeroMemory(m_InfoCarPathLinks, sizeof(m_InfoCarPathLinks));
    ZeroMemory(m_nConnectionWeight, sizeof(m_nConnectionWeight));
}
...
void CPathFind::StoreNodeInfoCar(int nInternalNodeCount, unsigned char iNodeType, signed char iNextNode, float fNodeX, float fNodeY, float fNodeZ, float fMedianWidth, unsigned char nLeftLanes, unsigned char nRightLanes, bool bIsIgnoredNode, bool bIsRestrictedAccess, unsigned char bSpeedLimit, bool bIsPoliceRoadBlock, unsigned char byteVehicleType, unsigned int dwSpawnRate, unsigned char bUnknown)
{
	...
	cptr_InfoForTileCars[c_nCarPathTiles].m_pathinfo[nInternalNodeCount].posCoors = CVector(fNodeX, fNodeY, fNodeZ) / 16.0f;
...
void CPathFind::StoreNodeInfoPed(int nInternalNodeCount, unsigned char iNodeType, signed char iNextNode, float fNodeX, float fNodeY, float fNodeZ, float fMedianWidth, unsigned char iunknown, bool bIsIgnoredNode, bool bIsRestrictedAccess, unsigned int dwSpawnRate) 
{
    ...
    cptr_InfoForTilePeds[c_nPedPathTiles].m_pathinfo[nInternalNodeCount].posCoors = CVector(fNodeX, fNodeY, fNodeZ) / 16.0f;
...
void CPathFind::Init(void)
{
    ...
    for (int i =0; i < (9650 * PATHMUL); i++)
       m_AttachedPaths[i].heuristicCost = 32766;
}
...
void CPathFind::AllocatePathFindInfoMem(void)
{
    ...
    memset(cptr_InfoForTileCars, 0, sizeof(cptr_InfoForTileCars));
    memset(cptr_InfoForTilePeds, 0, sizeof(cptr_InfoForTilePeds));
    memset(s_pTempExternalNodes, 0, sizeof(s_pTempExternalNodes));
    c_nPedPathTiles = 0;
    c_nCarPathTiles = 0;
}
...
void CPathFind::PreparePathDataForType( unsigned char bytePathDataFor, CTempNode* pTempNode, CPathInfoForObject* pUnusedPathInfos, float fUnkRange, CPathInfoForObject* pPathTiles, int nGroupNodesForObject) {
	...
	if (nGroupNodesForObject > 0)
	{
		for (int i = 0; i < nGroupNodesForObject; i++)
		{
			...
			for (int j = 0; j < 12; j++)
			{
				...
				{
					m_AttachedPaths[m_nAttachedNodes].StoreCoorWithPrecision(pPathTiles[i].m_pathinfo[j].posCoors);

					aGroupNodeIndex_InternalNode[m_nAttachedNodes] = i;
				...
				{
					s_pTempExternalNodes[nTempExternalNodes].posCoors = pPathTiles[i].m_pathinfo[j].posCoors;
    ...
    for (int i= 0; i < nTempExternalNodes; i++)
	{
		...
		for (int j = 0; j < nNewDetachedNode; j++)
		{
			...
			{
				CVector2D v2dNodeToExternalNode(pTempNode[j].posCoors - s_pTempExternalNodes[i].posCoors);

				v2dNodeToExternalNode.fX = abs(v2dNodeToExternalNode.fX);
				v2dNodeToExternalNode.fY = abs(v2dNodeToExternalNode.fY);

				//check if nodes are close enough 
				if (v2dNodeToExternalNode.fX <= fRange && v2dNodeToExternalNode.fY <= fRange)
                {
					// to check if more closer new-detached node exist
					if (v2dNodeToExternalNode.fX <= v2dNodeToExternalNode.fY)
                        fRange = v2dNodeToExternalNode.fY;
                    else
                        fRange = v2dNodeToExternalNode.fX;
		...
		{
			pTempNode[nNewDetachedNode].posCoors = s_pTempExternalNodes[i].posCoors;
		...
		else 
		{
			...
			CVector2D v2dDirectionToExternalNode = m_AttachedPaths[pTempNode[nClosestDetachedNodeToExternalNode].nConnectedAttachedNode].Form2DVector() - m_AttachedPaths[pTempNode[nClosestDetachedNodeToExternalNode].nOtherConnectedAttachedNode].Form2DVector();
			v2dDirectionToExternalNode.Normalize();
		
            pTempNode[nClosestDetachedNodeToExternalNode].byteNormalX = (signed char)(v2dDirectionToExternalNode.fX * 100.0f);
			pTempNode[nClosestDetachedNodeToExternalNode].byteNormalY = (signed char)(v2dDirectionToExternalNode.fY * 100.0f);

            //get the average of the two closest node
			pTempNode[nClosestDetachedNodeToExternalNode].posCoors += s_pTempExternalNodes[i].posCoors;
            pTempNode[nClosestDetachedNodeToExternalNode].posCoors /= 2.0f;
	...
	*	Number of nodes that is attached to one node is specified by
	...
	for (int i = nPrevObjectAttachedNodes; i < m_nAttachedNodes; i++)
	{
		...
		for (int j = 0; j < nNewDetachedNode; j++)
		{
			if (pTempNode[j].stateStoreType == CTempNode::AVERAGED_NODE)
			{
				...
				if (i == pTempNode[j].nConnectedAttachedNode || i == pTempNode[j].nOtherConnectedAttachedNode)
				{
					...
					float fDistance = (m_AttachedPaths[i].Form3DVector() - pCurrentNode->Form3DVector()).Length();
					...
					if (bytePathDataFor == PATHDATAFOR_CAR)
					{
                        ...
                        for (int k = 0; k < m_nCarPathLinks; k++)
						{
							if (
								...
								m_CarPathLinks[k].wX == (signed int)(8.0f * pTempNode[j].posCoors.fX) &&
								m_CarPathLinks[k].wY == (signed int)(8.0f * pTempNode[j].posCoors.fY) 
							)
							{
						...
						if (!bIsDetachedNodeAlreadyStored)
						{
							...
							m_CarPathLinks[m_nCarPathLinks].wX = (signed int)(8.0f * pTempNode[j].posCoors.fX);
							m_CarPathLinks[m_nCarPathLinks].wY = (signed int)(8.0f * pTempNode[j].posCoors.fY);
		...
		CPathInfoForObject *p_iPathTile = &pPathTiles[aGroupNodeIndex_InternalNode[i]];
        int iPathTileOffset = 0;
		...
		// get the offset of i in the path group
		for (int k = nAssumedGroupFirstNode; k < i; k++)
			if (aGroupNodeIndex_InternalNode[i] == aGroupNodeIndex_InternalNode[k]) 
				iPathTileOffset++;
		...
		for(int nNodesInCurrentGroup = nAssumedGroupFirstNode; nNodesInCurrentGroup < nAssumedGroupLastNode; nNodesInCurrentGroup++)
        {
			...
			if (aGroupNodeIndex_InternalNode[i] == aGroupNodeIndex_InternalNode[nNodesInCurrentGroup])
			{
				if (nNodesInCurrentGroup != i){
					int currentOffsetInGroup = iPathTileOffset + nNodesInCurrentGroup - i; //I had  written 1 instead of i
					if (currentOffsetInGroup == p_iPathTile->m_pathinfo[iPathTileOffset].sbNextNode ||
						iPathTileOffset == p_iPathTile->m_pathinfo[currentOffsetInGroup].sbNextNode)
                    {
						...
						{
							if (currentOffsetInGroup == p_iPathTile->m_pathinfo[iPathTileOffset].sbNextNode && //NOT FEELING WELL HERE
								p_iPathTile->m_pathinfo[iPathTileOffset].byteFlags & NODEINFOFLAGS_ISCROSSROAD ||
							    iPathTileOffset == p_iPathTile->m_pathinfo[currentOffsetInGroup].sbNextNode &&
							    p_iPathTile->m_pathinfo[currentOffsetInGroup].byteFlags & NODEINFOFLAGS_ISCROSSROAD)
						    {
    ...
    if (bytePathDataFor == PATHDATAFOR_PED){
        while (nPrevObjectAttachedNodes < m_nAttachedNodes){
            if (m_AttachedPaths[nPrevObjectAttachedNodes].bitnumberOfNodesConnected == 0){
                ...
                for (int n = nPrevObjectAttachedPoints; n < (int)m_nConnectedNodes;n++){
...
int CPathFind::FindNodeClosestToCoors(CVector& posCoors, unsigned char iPathDataFor, float fRangeCoefficient, bool bCheckIgnored, bool bCheckRestrictedAccess, bool bCheckUnkFlagFor2, bool bIsVehicleBoat){
	...
	for (int i = iStartNodeIndex; i < iEndNodeIndex; i++)
	{
		...
		{
			float fXDiff = abs(((float)pNode->wX / 8.0f) - posCoors.fX);
			float fYDiff = abs(((float)pNode->wY / 8.0f) - posCoors.fY);
			float fZDiff = abs(((float)pNode->wZ / 8.0f) - posCoors.fZ);
...
void CPathFind::FindNextNodeWandering(uint8_t iPathDataFor, CVector& posCoors, CPathNode** pCurrentNode, CPathNode** pNextNode, uint8_t u8QuartersRotation, uint8_t *byteNewDirection){
    ...
    // get normal vectors along with translation of u8QuartersRotation which is limited to maximum value of 8
    // optimization is needed so as to directly use floating point assembly function
    double dPrevNormalX = sin((double)u8QuartersRotation * M_PI/4.0);
    double dPrevNormalY = cos((double)u8QuartersRotation * M_PI/4.0);
    ...
    *pNextNode = NULL;
    ...
    for (int i = 0; i < pCurrentActPedNode->bitnumberOfNodesConnected; i++)
	{
        ...
        // Don't go from unignored node to ignored node
		if (!(pCurrentActPedNode->bitIsIgnoredNode) && pNextConnectedNode->bitIsIgnoredNode)
			continue;
		
		CVector posNextNode = pNextConnectedNode->Form3DVector();
		posNextNode.fZ += 1.0f;

		// Never make a ped go through obstacles
        if (!CWorld::GetIsLineOfSightClear(&vecCurrentPos, &posNextNode, 1, 0, 0, 0, 0, 0, 0))
			continue;
		
				
		CVector2D v2dUnitDelta(posNextNode);
				
		v2dUnitDelta.Normalize();

        double fUnitLengthSq = v2dUnitDelta.DotProduct(CVector2D(dPrevNormalX, dPrevNormalY));
        if (fUnitLengthSq >= (double)fPrevFoundRangeCoeff)
		{
            ...
            *pNextNode = pNextConnectedNode;
			...
			{
				double fabsDeltaNormalY = abs(v2dUnitDelta.fY);
...
bool CPathFind::NewGenerateCarCreationCoors(CVector2D& pos2DCoors, CVector2D& v2dDirection, float fRange, float fZlookUp, bool bShouldSpawnPositiveDirection, CVector *pVecPosition, int *aMainNodeIndex, int *aSubNodeIndex, float *aNodeRangeDiffCoeff, char bDontCheckIgnored) {
	...
	for(int i = 0; i < (500 * PATHMUL); i++) 
	{
		...
		if((m_AttachedPaths[nRandIndex].bitIsIgnoredNode) && bDontCheckIgnored == false) 
			continue;
		...
		for(int j = 0; j < m_AttachedPaths[nRandIndex].bitnumberOfNodesConnected; j++) 
		{
			...
			if((m_AttachedPaths[nNextNodeIndex].bitIsIgnoredNode) && bDontCheckIgnored == false) 
				continue;
			...
			float fabsRandNodeRangeDelta = abs(frandNodeRangeDelta);
            float fabsNextNodeRangeDelta = abs(fnextNodeRangeDelta);
	    ...
	    for (int j = 0; j < nConnectedNodeSets; j++) 
		{
    		    ...
    		    (fStartNodeDisplacement > fSecMinRange || fNextNodeDisplacement > fSecMinRange))
		    {
			    for (int k = 0; k < 5; k++) {
					...
					RwV3d& vecMeanPos = *(RwV3d*)&vTemp; //WHAT UGLY DUCKINGL
				    ...
				    if (rwTransformationMatrix == NULL) 
    					RwV3dTransformPoints(&vecTransform, &vecTransform, 1, (RwMatrix*)&TheCamera.matView);
				    ...
				    if ((vecTransform.y + 2.0f) >= CGameVariables::GetNearClipZ() &&
					    ...
					    (vecTransform.y * (*(CCameraJPT*)(&TheCamera)).field_8B8.y + vecTransform.x * (*(CCameraJPT*)(&TheCamera)).field_8B8.x) <= 2.0f &&
					    (vecTransform.y * (*(CCameraJPT*)(&TheCamera)).field_8C4.y + vecTransform.x * (*(CCameraJPT*)(&TheCamera)).field_8C4.x) <= 2.0f &&
					    (vecTransform.z * (*(CCameraJPT*)(&TheCamera)).field_8D0.z + vecTransform.y * (*(CCameraJPT*)(&TheCamera)).field_8D0.y) <= 2.0f &&
    					(vecTransform.z * (*(CCameraJPT*)(&TheCamera)).field_8DC.z + vecTransform.y * (*(CCameraJPT*)(&TheCamera)).field_8DC.y) <= 2.0f)
				    {
						...
						(fRandNodeDisplacement > fMinRange && fRandNodeDisplacement < fMaxRange))
				    {
					    ...
					    *pNextNodeIndex = nNextNodeIndex;
					    ...
					    if (abs(fSpawnPosCollisionGround - vecMeanPos.z) <= 3.0f) {
...
float CPathFind::CalcRoadDensity(CVector2D& pos2dCoors) {
	...
	for (int i = 0; i < m_nCarAttachedNodes; i++) 
	{
		...
		if (abs(v2dStartNodeToCoors.fX) >= 80.0f)
			continue;
		
		if (abs(v2dStartNodeToCoors.fY) >= 80.0f)
			continue;
...
int CPathFind::FindNodeClosestToCoorsFavourDirection(CVector& posCoors, uint8_t uiPathDataFor, CVector2D& v2dPreferedDirection) {
	...
	for (int i = iStartNodeIndex; i < iEndNodeIndex; i++) 
	{
		...
		float fcurDistCoefficient = abs(vecStartNodeToCoors.fX) + abs(vecStartNodeToCoors.fY) + 3.0f* abs(vecStartNodeToCoors.fZ);
...
{
    static CPathNode* pIntermediateCarRouteInfos[32] = {nullptr};
...
float CCarPathLink::OneWayLaneOffset() 
{
	...
	if (this->bitLeftLanes && this->bitRightLanes)
		fdistance = (float)(this->sbMedianWidth) / 80.0f + 0.5f;
	else if(this->bitLeftLanes)
		fdistance = 0.5f - (float)(this->bitLeftLanes) * 0.5f;
	else
		fdistance = 0.5f - (float)(this->bitRightLanes) * 0.5f;
	return fdistance;
}
