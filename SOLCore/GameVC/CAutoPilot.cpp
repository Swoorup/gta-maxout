...
void CAutoPilot::ModifySpeed(float fModSpeed) {
	...
	vecCurrentDirection.fX = (float)m_byteCurrentDirectionScale * (float)(CGameVariables::GetPathFind()->m_DetachedNodes[m_dwCurrentDetachedNodeIndex].NormalVecX) * fDetachedPathNormalVecMinifier;
	vecCurrentDirection.fY = (float)m_byteCurrentDirectionScale * (float)(CGameVariables::GetPathFind()->m_DetachedNodes[m_dwCurrentDetachedNodeIndex].NormalVecY) * fDetachedPathNormalVecMinifier;
	vecNextDirection.fX = (float)m_byteNextDirectionScale * (float)(CGameVariables::GetPathFind()->m_DetachedNodes[m_dwNextDetachedNodeIndex].NormalVecX) * fDetachedPathNormalVecMinifier;
	vecNextDirection.fY = (float)m_byteNextDirectionScale * (float)(CGameVariables::GetPathFind()->m_DetachedNodes[m_dwNextDetachedNodeIndex].NormalVecY) * fDetachedPathNormalVecMinifier;
	
	float fCurrentLaneLength = ((float)m_byteCurrentLanes + 0.5f) * 5.0f;
	float fNextLaneLength = ((float)m_byteNextLanes + 0.5f) * 5.0f;
	vecCurrentPoint.fX = (float)(CGameVariables::GetPathFind()->m_DetachedNodes[m_dwCurrentDetachedNodeIndex].wX) / 8.0f + vecCurrentDirection.fY * fCurrentLaneLength;
	vecCurrentPoint.fY = (float)(CGameVariables::GetPathFind()->m_DetachedNodes[m_dwCurrentDetachedNodeIndex].wY) / 8.0f - vecCurrentDirection.fX * fCurrentLaneLength;
	vecNextPoint.fX = (float)(CGameVariables::GetPathFind()->m_DetachedNodes[m_dwNextDetachedNodeIndex].wX) / 8.0f + vecNextDirection.fY * fNextLaneLength;
	vecNextPoint.fY = (float)(CGameVariables::GetPathFind()->m_DetachedNodes[m_dwNextDetachedNodeIndex].wY) / 8.0f - vecNextDirection.fX * fNextLaneLength;
