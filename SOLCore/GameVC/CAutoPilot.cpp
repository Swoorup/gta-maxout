...
void CAutoPilot::ModifySpeed(float fModSpeed) {
	...
	vecCurrentDirection.fX = (float)m_byteCurrentDirectionScale * (float)(CGameVariables::GetPathFind()->m_CarPathLinks[m_dwCurrentCarLinkNode].NormalVecX) * fDetachedPathNormalVecMinifier;
	vecCurrentDirection.fY = (float)m_byteCurrentDirectionScale * (float)(CGameVariables::GetPathFind()->m_CarPathLinks[m_dwCurrentCarLinkNode].NormalVecY) * fDetachedPathNormalVecMinifier;
	vecNextDirection.fX = (float)m_byteNextDirectionScale * (float)(CGameVariables::GetPathFind()->m_CarPathLinks[m_dwNextCarLinkNode].NormalVecX) * fDetachedPathNormalVecMinifier;
	vecNextDirection.fY = (float)m_byteNextDirectionScale * (float)(CGameVariables::GetPathFind()->m_CarPathLinks[m_dwNextCarLinkNode].NormalVecY) * fDetachedPathNormalVecMinifier;
	
	float fCurrentLaneLength = ((float)m_byteCurrentLanes + 0.5f) * 5.0f;
	float fNextLaneLength = ((float)m_byteNextLanes + 0.5f) * 5.0f;
	vecCurrentPoint.fX = (float)(CGameVariables::GetPathFind()->m_CarPathLinks[m_dwCurrentCarLinkNode].wX) / 8.0f + vecCurrentDirection.fY * fCurrentLaneLength;
	vecCurrentPoint.fY = (float)(CGameVariables::GetPathFind()->m_CarPathLinks[m_dwCurrentCarLinkNode].wY) / 8.0f - vecCurrentDirection.fX * fCurrentLaneLength;
	vecNextPoint.fX = (float)(CGameVariables::GetPathFind()->m_CarPathLinks[m_dwNextCarLinkNode].wX) / 8.0f + vecNextDirection.fY * fNextLaneLength;
	vecNextPoint.fY = (float)(CGameVariables::GetPathFind()->m_CarPathLinks[m_dwNextCarLinkNode].wY) / 8.0f - vecNextDirection.fX * fNextLaneLength;
