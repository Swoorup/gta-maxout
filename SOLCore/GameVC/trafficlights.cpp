...
uint8_t CTrafficLights::LightsForCars1(void)
{
	...
	int nTime = (CTimer::m_snTimeInMillisecondsPx >> 1) & 0x3FFF;
	if (nTime <= 4999)
		return TL_GREENLIGHT;
	else
		if (nTime > 5999)
...
{
	CCarPathInfoLinkIndex nLinkNext = pVeh->_.stAutopilot.m_dwNextCarLinkNode;
	CCarPathInfoLinkIndex nLinkCurrent = pVeh->_.stAutopilot.m_dwCurrentCarLinkNode;
	CCarPathInfoLinkIndex nLinkPrev = pVeh->_.stAutopilot.m_dwPrevCarLinkNode;
	...
	{
		if( !lightStateAtCurrentLink || !bIsLightEnAtCurrentConnection && nLinkCurrent.IndexToAttachedNode() != pVeh->_.stAutopilot.m_dwCurrentNode || bIsLightEnAtCurrentConnection && nLinkCurrent.IndexToAttachedNode() == pVeh->_.stAutopilot.m_dwCurrentNode)
		{
			if((pVeh->_.phys.ent.bfTypeStatus >> 3) != 3 || !lightStateAtPrevLink || !bIsLightEnAtPrevConnection && nLinkPrev.IndexToAttachedNode() != pVeh->_.stAutopilot.m_dwPrevNode || bIsLightEnAtPrevConnection && nLinkPrev.IndexToAttachedNode() == pVeh->_.stAutopilot.m_dwPrevNode)
				return false;
			else
			{
				if(!bCheckSignal)
				{
					if(lightStateAtPrevLink != 1 || !LightsForCars1())
					{
						...
						CVector2D v2dToPrevNode = CVector2D(pVeh->_.phys.ent.mat.vPos) - pThePaths->m_CarPathLinks[nLinkPrev.n].GetPos();
						...
						if(pVeh->_.stAutopilot.m_bytePrevDirectionScale == -1)
						{
	...
	if(!( !lightStateAtNextLink || !bIsLightEnAtNextConnection && nLinkNext.IndexToAttachedNode() != pVeh->_.stAutopilot.m_dwNextNode || bIsLightEnAtNextConnection && nLinkNext.IndexToAttachedNode() == pVeh->_.stAutopilot.m_dwNextNode))
	{
		if(!bCheckSignal)
		{
		...
		CVector2D v2dToNextNode = CVector2D(pVeh->_.phys.ent.mat.vPos) - pThePaths->m_CarPathLinks[nLinkNext.n].GetPos();
		...
		if(pVeh->_.stAutopilot.m_byteNextDirectionScale == -1)
		{
	...
	currentCondition:if(!( !lightStateAtCurrentLink || !bIsLightEnAtCurrentConnection && nLinkCurrent.IndexToAttachedNode() != pVeh->_.stAutopilot.m_dwCurrentNode || bIsLightEnAtCurrentConnection && nLinkCurrent.IndexToAttachedNode() == pVeh->_.stAutopilot.m_dwCurrentNode))
	{
		if(!bCheckSignal)
		{
		...
		CVector2D v2dToCurrentNode = CVector2D(pVeh->_.phys.ent.mat.vPos) - pThePaths->m_CarPathLinks[nLinkCurrent.n].GetPos();
		...
		if(pVeh->_.stAutopilot.m_byteCurrentDirectionScale == -1)
		{
...
void CTrafficLights::ScanForLightsOnMapN(void)
{
	for each (CSector itr in CWorld::stSectorList.n)
	{
		...
		while (pListTraverse.pStart)
		{
			...
			posLSigInWorld = pEnt->_.mat * vecLeftSig;
			posRSigInWorld = pEnt->_.mat * vecRightSig;
			...
			for (int nNode = 0; nNode < pThePaths->m_nAttachedNodes; nNode++)
			{
				...
				for(int nNodesConnectedOffset = 0; nNodesConnectedOffset < ((CPathNodeIndex)nNode).NumOfAttachedNodes(); nNodesConnectedOffset++)
				{
					...
					if ((CPathNodeIndex)nNode < nAttachedNode)
					{
						...
						if (fAvgHeight < 10.0f)
						{
							...
							if(bBastard)
							{
								...
								float frad = CGeneral::GetATanOfXY(CVector2D(pEnt->_.mat.vLookAt));
			...
			{
				CVector2D v2dPedNodeToEntPos = pThePaths->m_AttachedPaths[nPedNode].Form2DVector() - pEnt->_.mat.vPos;
				...
				for (int nNodesConnectedOffset = 0; nNodesConnectedOffset < ((CPathNodeIndex)nPedNode).NumOfAttachedNodes(); nNodesConnectedOffset++)
				{
					...
					{
						CPathNodeIndex nAttachedNode = GetAttachedNodeFromArrayIndex(nArrayInfoIndex);
						CVector2D v2dPedAttachedNodeToEntPos = pThePaths->m_AttachedPaths[nAttachedNode.n].Form2DVector();
